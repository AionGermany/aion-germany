	...
	private void checkEntryConditions(Player player, int dialogId, int questId) {
		if (dialogId == 10000) {
			if (player.getInventory().decreaseByItemId(185000136, 1)) {
				...
				PacketSendUtility.sendBrightYellowMessageOnCenter(player, "You can enter the Eternal Bastion!");
				stopMove(player);
				AI2Actions.deleteOwner(this);
			}
			else {
				PacketSendUtility.sendBrightYellowMessageOnCenter(player, "You cannot enter the Eternal Bastion!");
