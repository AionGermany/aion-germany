 ...
 * @author Alex
 */
public class NetworkBan extends AdminCommand {
	...
	public void execute(Player player, String... params) {
		...
		if (params.length > 1) {
			...
			if (target != null && target instanceof Player) {
				...
				ip = targetpl.getClientConnection().getIP();
				targetName = targetpl.getName();
				targetpl.getClientConnection().closeNow();
			}
			else {
				onFail(player, "You should select a player or give me any network address");
				return;
			}
		}

		NetworkBannedManager.getInstance().banAddress(ip, System.currentTimeMillis() + time * 60 * 1000, "author=" + player.getName() + ", " + player.getObjectId() + "; target=" + targetName);
	}
	...
	public void onFail(Player player, String message) {
		...
		PacketSendUtility.sendMessage(player, "Syntax: //netban [time in minutes] <network address>");
