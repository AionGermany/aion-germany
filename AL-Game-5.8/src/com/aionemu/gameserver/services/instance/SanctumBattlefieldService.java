 ...
 * @author Falke_34
 */
public class SanctumBattlefieldService {

	/*
	 * Used to logs information.
	 */
	private static final Logger log = LoggerFactory.getLogger(SanctumBattlefieldService.class);
	...
	// MaskId for Sanctum Battlefield Instance.
	public static final int maskId = 416;

	public static final int InstanceMapId = 302200000;
	...
	private static class SingletonHolder {
		...
		protected static final SanctumBattlefieldService instance = new SanctumBattlefieldService();
	}

	public static SanctumBattlefieldService getInstance() {
		return SingletonHolder.instance;
	}

	/**
	 * Schedules the Sanctum Battlefield Instance to be launched at the specified Cron-Time.
	 */
	public void start() {
		String[] times = AutoGroupConfig.SANCTUMBATTLEFIELD_TIMES.split("\\|");
		for (String cron : times) {
			...
			log.info("Scheduled Sanctum Battlefield Instance based on cron expression: " + cron + " Duration: " + AutoGroupConfig.SANCTUMBATTLEFIELD_TIMER + " in minutes");
		}
	}
		...
		while (iter.hasNext()) {
			...
			if (player.getLevel() > minlevel && player.getLevel() < maxlevel) {
				if (!isInInstance(player)) {
					...
					// You can now participate in the Sanctum Battlefield battle.
	...
	private void ScheduleUnregistration() {
		...
		}, AutoGroupConfig.SANCTUMBATTLEFIELD_TIMER * 60 * 1000);
	}
	...
	public boolean isSanctumBattlefieldAvailable() {
