...
public class _50008SoloriusShugoSlackers extends QuestHandler {
	...
	public void register() {
		qe.registerQuestNpc(831038).addOnQuestStart(questId); // Smootinerk
		qe.registerQuestNpc(831038).addOnTalkEvent(questId);
		qe.registerQuestNpc(219290).addOnAddAggroListEvent(questId); // Lazy Courier Shugo
	...
	public boolean onAddAggroListEvent(QuestEnv env) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			if (var == 0 || var == 1) {
				return defaultStartFollowEvent(env, (Npc) env.getVisibleObject(), ZoneName.get("Q50008"), 0, 0);
			}
		}
		return false;
	}
