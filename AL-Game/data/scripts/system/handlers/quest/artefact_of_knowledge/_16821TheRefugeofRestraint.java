 ...
 * @author Phantom_KNA
 */
public class _16821TheRefugeofRestraint extends QuestHandler {

	public _16821TheRefugeofRestraint() {
		super(questId);
	}
	...
	public void register() {
		...
		qe.registerQuestNpc(806281).addOnTalkEvent(questId); // Benio
		qe.registerQuestNpc(806426).addOnTalkEvent(questId); // Burgos
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (qs.getStatus() == QuestStatus.START) {
			...
			else if (targetId == 806426) { // Burgos
				switch (dialog) {
					case QUEST_SELECT: {
						if (var == 1) {
	...
	public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			if (var == 4 && zoneName == ZoneName.get("REFUGE_OF_RESTRAINT_301550000")) {
