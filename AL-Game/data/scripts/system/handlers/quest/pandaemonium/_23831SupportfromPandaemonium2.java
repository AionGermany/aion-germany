 ...
 * @author FrozenKiller
 */
public class _23831SupportfromPandaemonium2 extends QuestHandler {

	private final static int questId = 23831;

	public _23831SupportfromPandaemonium2() {
		super(questId);
	}

	@Override
	public void register() {
		qe.registerQuestItem(182216124, questId);
	...
	public boolean onEnterWorldEvent(QuestEnv env) { // Fix for player who already have this Quest
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.START && player.getInventory().getItemCountByItemId(182216124) < 1 && player.getWorldId() == 120010000) {
			return giveQuestItem(env, 182216124, 1);
		}
		return false;
	}
}
