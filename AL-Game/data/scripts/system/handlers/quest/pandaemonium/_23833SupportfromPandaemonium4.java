 ...
 * @author FrozenKiller
 */
public class _23833SupportfromPandaemonium4 extends QuestHandler {

	private final static int questId = 23833;

	public _23833SupportfromPandaemonium4() {
		super(questId);
	}

	@Override
	public void register() {
		qe.registerQuestItem(182216126, questId);
		qe.registerOnLevelUp(questId);
		qe.registerQuestNpc(204061).addOnTalkEvent(questId);
	...
	public boolean onEnterWorldEvent(QuestEnv env) { // Fix for player who already have this Quest
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.START && player.getInventory().getItemCountByItemId(182216126) < 1 && player.getWorldId() == 120010000) {
			return giveQuestItem(env, 182216126, 1);
		}
		return false;
	}
}
