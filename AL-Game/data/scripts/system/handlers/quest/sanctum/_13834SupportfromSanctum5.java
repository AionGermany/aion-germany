 ...
 * @author FrozenKiller
 */
public class _13834SupportfromSanctum5 extends QuestHandler {

	private final static int questId = 13834;

	public _13834SupportfromSanctum5() {
		super(questId);
	}

	@Override
	public void register() {
		qe.registerQuestItem(182216122, questId);
	...
	public boolean onLvlUpEvent(QuestEnv env) {
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (player.getLevel() >= 55 && (qs == null || qs.getStatus() == QuestStatus.NONE) && player.getRace() == Race.ELYOS) {
			giveQuestItem(env, 182216122, 1);
	...
	public boolean onEnterWorldEvent(QuestEnv env) { // Fix for player who already have this Quest
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.START && player.getInventory().getItemCountByItemId(182216122) < 1 && player.getWorldId() == 110010000) {
			return giveQuestItem(env, 182216122, 1);
		}
		return false;
	}
}
