...
public class _30760PetrifiedHeroOfTheAsmodians extends QuestHandler {
	...
	public void register() {
		qe.registerQuestNpc(800070).addOnQuestStart(questId);
		for (int npc : npcs) {
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 701499) {
				QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 800383, player.getX(), player.getY(), player.getZ(), (byte) 0);
