...
public class MySQL5PlayerEventsWindowDAO extends PlayerEventsWindowDAO {
	...
	public PlayerEventWindowList load(Player player) {
		List<PlayerEventWindowEntry> ew = new ArrayList<PlayerEventWindowEntry>();
		Connection con = null;
		try {
			...
			while (rset.next()) {
				...
				ew.add(new PlayerEventWindowEntry(id, lastStamp, elapsed, PersistentState.UPDATED));
			}
			rset.close();
			stmt.close();
		} catch (Exception e) {
		...
		return new PlayerEventWindowList(ew);
	}
	...
	public void delete(final int accountId, final int eventId) {
		DB.insertUpdate("DELETE FROM player_events_window WHERE account_id = ? AND event_id = ?",
                new IUStH() {
                    @Override
	...
	public List<Integer> getEventsWindow(final int accountId) {
        ...
        DB.select("SELECT event_id FROM player_events_window WHERE account_id = ?",
                new ParamReadStH() {
                    @Override
