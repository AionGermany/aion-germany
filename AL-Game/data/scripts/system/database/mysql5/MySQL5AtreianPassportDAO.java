...
public class MySQL5AtreianPassportDAO extends AtreianPassportDAO {
	...
	public void insertPassport(final int accountId, final int passportId, final int stamps, final Timestamp last_stamp) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement("INSERT INTO `atreian_passports` (`account_id`, `passport_id`, `stamps`, `last_stamp`) VALUES (?,?,?,?)");
	...
	public void updatePassport(final int accountId, final int passportId, final int stamps, final boolean rewarded, final Timestamp last_stamp) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE atreian_passports SET stamps = ?, rewarded = ?, last_stamp = ? WHERE account_id = ? AND passport_id = ?");
	...
	public int getStamps(final int accountId, final int passportId) {
		PreparedStatement s = DB.prepareStatement("SELECT * FROM atreian_passports WHERE account_id = ? AND passport_id = ?");
		try {
	...
	public Timestamp getLastStamp(final int accountId, final int passportId) {
		PreparedStatement s = DB.prepareStatement("SELECT last_stamp FROM atreian_passports WHERE account_id = ? AND passport_id = ?");
		try {
			s.setInt(1, accountId);
			s.setInt(2, passportId);
			ResultSet rs = s.executeQuery();
