...
$bef = "SELECT gp, all_kill, rank, id, name, exp, race, gender, player_class FROM abyss_rank LEFT JOIN players ON player_id = id WHERE gp>1 ORDER BY ap DESC LIMIT 100";
...
while ($row = $sql_gs->fetch_assoc($res))
{
	...
	echo '<td class="toptext">'.number_format($gp,0, ' ','.').'</td>';
