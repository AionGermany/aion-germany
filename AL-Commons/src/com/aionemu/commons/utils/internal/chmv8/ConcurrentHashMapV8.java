 ...
 * in a way that prevents all access. This class is fully inter-operable with
 ...
 * iterators are designed to be used by only one thread at a time. Bear in mind
 * that the results of aggregate status methods including {@code size}, {@code
 * isEmpty}, and {@code containsValue} are typically useful only when a map is
 * not undergoing concurrent updates in other threads. Otherwise the results of
 * these methods reflect transient states that may be adequate for monitoring or
 ...
 * size estimate as an optional {@code
 * initialCapacity} constructor argument. An additional optional
 * {@code loadFactor} constructor argument provides a further means of
 * customizing initial table capacity by specifying the table density to be used
 * in calculating the amount of space to allocate for the given number of
 * elements. Also, for compatibility with previous versions of this class,
 * constructors may optionally specify an expected {@code concurrencyLevel} as
 * an additional hint for internal sizing. Note that using many keys with
 * exactly the same {@code hashCode()} is a sure way to slow down performance of
 * any hash table.
 *
 * <p>
 * A {@link Set} projection of a ConcurrentHashMapV8 may be created (using
 ...
 *
 * <p>
 * A ConcurrentHashMapV8 can be used as scalable frequency map (a form of
 * histogram or multiset) by using {@link LongAdder} values and initializing via
 * {@link #computeIfAbsent}. For example, to add a count to a
 * {@code ConcurrentHashMapV8<String,LongAdder> freqs}, you can use
 * {@code freqs.computeIfAbsent(k -> new
 * LongAdder()).increment();}
 *
 * <p>
 ...
 *
 * <p>
 * ConcurrentHashMapV8s support sequential and parallel operations bulk
 * operations. (Parallel forms use the {@link ForkJoinPool#commonPool()}). Tasks
 * that may be used in other contexts are available in class
 * {@link ForkJoinTasks}. These operations are designed to be safely, and often
 * sensibly, applied even with maps that are being concurrently updated by other
 * threads; for example, when computing a snapshot summary of the values in a
 * shared registry. There are three kinds of operation, each with four forms,
 * accepting functions with Keys, Values, Entries, and (Key, Value) arguments
 * and/or return values. Because the elements of a ConcurrentHashMapV8 are not
 * ordered in any particular way, and may be processed in different orders in
 * different parallel executions, the correctness of supplied functions should
 * not depend on any ordering, or on any other objects or values that may
 * transiently change while computation is in progress; and except for forEach
 ...
 * function on each element; skipping further search when a result is
 * found.</li>
 *
 ...
 * to each element.</li>
 *
 * <li>Reductions to scalar doubles, longs, and ints, using a given basis
 * value.</li>
 *
 * </li>
 * </ul>
 ...
 * ConcurrentHashMapV8: Any non-null result returned from {@code get(key)} and
 ...
 *
 * <p>
 * <em>jsr166e note: During transition, this class uses nested functional
 * interfaces with different names but the same forms as those expected for
 * JDK8.</em>
 *
 * <p>
...
public class ConcurrentHashMapV8<K, V> implements ConcurrentMap<K, V>, Serializable {
	...
	/**
	 * A partitionable iterator. A Spliterator can be traversed directly, but
	 * can also be partitioned (before traversal) by creating another
	 * Spliterator that covers a non-overlapping portion of the elements, and so
	 * may be amenable to parallel execution.
	 *
	 * <p>
	 * This interface exports a subset of expected JDK8 functionality.
	 *
	 * <p>
	 * Sample usage: Here is one (of the several) ways to compute the sum of the
	 * values held in a map using the ForkJoin framework. As illustrated here,
	 * Spliterators are well suited to designs in which a task repeatedly splits
	 * off half its work into forked subtasks until small enough to process
	 * directly, and then joins these subtasks. Variants of this style can also
	...
	public static interface Spliterator<T> extends Iterator<T> {
		 ...
		 * Spliterator. After invoking this method, the current Spliterator will
		 * <em>not</em> produce any of the elements of the returned Spliterator,
		 * but the two Spliterators together will produce all of the elements
		 * that would have been produced by this Spliterator had this method not
		 * been called. The exact number of elements produced by the returned
		 * Spliterator is not guaranteed, and may be zero (i.e., with
		 * {@code hasNext()} reporting {@code
		 * false}) if this Spliterator cannot be further split.
		 *
		 * @return a Spliterator covering approximately half of the elements
		 * @throws IllegalStateException
		 *             if this Spliterator has already commenced traversing
		 *             elements
		 */
		Spliterator<T> split();
	}
	 ...
	 *
	 * Each key-value mapping is held in a Node. Because Node key fields can
	 * contain special values, they are defined using plain Object types (not
	 * type "K"). This leads to a lot of explicit casting (and many explicit
	 * warning suppressions to tell compilers not to complain about it). It also
	 * allows some of the public methods to be factored into a smaller number of
	 * internal methods (although sadly not so for the five variants of
	 * put-related operations). The validation-based approach explained below
	 * leads to a lot of code sprawl because retry-control precludes factoring
	 ...
	 * (sun.misc.Unsafe) operations. The lists of nodes within bins are always
	 * accurately traversable under volatile reads, so long as lookups check
	 ...
	 * negative hash fields are forwarding nodes to either TreeBins or resized
	 * tables. The lower 31 bits of each normal Node's hash field contain a
	 * transformation of the key's hash code.
	 *
	 ...
	 * resizing). However, operations that only conditionally update may inspect
	 * nodes until the point of update. This is a converse of sorts to the lazy
	 * locking technique described by Herlihy & Shavit.
	 *
	 ...
	 * in which multiple keys are designed to have identical hash codes. Also,
	 * although we guard against the worst effects of this (see method spread),
	 * sets of hashes may differ only in bits that do not impact their bin index
	 * for a given power-of-two mask. So we use a secondary strategy that
	 * applies when the number of nodes in a bin exceeds a threshold, and at
	 * least one of the keys implements Comparable. These TreeBins use a
	 * balanced tree to hold nodes (a specialized form of red-black trees),
	 * bounding search time to O(log N). Each search step in a TreeBin is around
	 * twice as slow as in a regular list, but given that N cannot exceed
	 * (1<<64) (before running out of addresses) this bounds search steps, lock
	 * hold times, etc, to reasonable constants (roughly 100 nodes inspected per
	 * operation worst case) so long as keys are Comparable (which is very
	 * common -- String, Long, etc). TreeBin nodes (TreeNodes) also maintain the
	 * same "next" traversal pointers as regular nodes, so can be traversed in
	 ...
	 * table. To enable concurrency, the next table must be (incrementally)
	 * prefilled with place-holders serving as reverse forwarders to the old
	 * table. Because we are using power-of-two expansion, the elements from
	 * each bin must either stay at same index, or move with a power of two
	 * offset. We eliminate unnecessary node creation by catching cases where
	 * old nodes can be reused because their next fields won't change. On
	 * average, only about one-sixth of them need cloning when a table doubles.
	 * The nodes they replace will be garbage collectable as soon as they are no
	 * longer referenced by any reader thread that may be in the midst of
	 * concurrently traversing table. Upon transfer, the old table bin contains
	 * only a special forwarding node (with hash field "MOVED") that contains
	 * the next table as its key. On encountering a forwarding node, access and
	 * update operations restart, using the new table.
	 *
	 ...
	 * traversal. This is arranged by proceeding from the last bin (table.length
	 * - 1) up towards the first. Upon seeing a forwarding node, traversals (see
	 * class Traverser) arrange to move to the new table without revisiting
	 * nodes. However, to ensure that no intervening nodes are skipped, bin
	 * splitting can only begin after the associated reverse-forwarders are in
	 * place.
	 *
	 ...
	 * threshold (and after resizing, many fewer do so). The bulk putAll
	 * operation further reduces contention by only committing count updates
	 * upon these size checks.
	 *
	 ...
	 * of in-progress next table while resizing. Uses are null checked by
	 * callers, and implicitly bounds-checked, relying on the invariants that
	 * tab arrays have non-zero size, and all indices are masked with
	 * (tab.length - 1) which is never negative and always less than length.
	 * Note that, to be correct wrt arbitrary concurrency errors by users,
	 * bounds checks must operate on local variables, which accounts for some
	 * odd-looking inline assignments below.
	 */
	@SuppressWarnings("unchecked")
	static final <V> Node<V> tabAt(Node<V>[] tab, int i) { // used by Traverser
	...
	/* ---------------- Nodes -------------- */
	/**
	 * Key-value entry. Note that this is never exported out as a user-visible
	 * Map.Entry (see MapEntry below). Nodes with a hash field of MOVED are
	 * special, and do not contain user keys or values. Otherwise, keys are
	 * never null, and null val fields indicate that a node is in the process of
	 * being deleted or created. For purposes of read-only access, a key may be
	 * read before a val, but can only be used after checking val to be
	 * non-null.
	 */
	static class Node<V> {
	...
	/**
	 * A specialized form of red-black tree for use in bins whose size exceeds a
	 * threshold.
	 *
	 ...
	 * Comparable<T> for the same T, so we cannot invoke compareTo among them.
	 * To handle this, the tree is ordered primarily by hash value, then by
	 * getClass().getName() order, and then by Comparator order among elements
	 * of the same class. On lookup at a node, if elements are not comparable or
	 * compare as 0, both left and right children may need to be searched in the
	 * case of tied hash values. (This corresponds to the full list search that
	 * would be necessary if all elements were non-Comparable and had tied
	 * hashes.) The red-black balancing code is updated from pre-jdk-collections
	 ...
	 *
	 * TreeBins also maintain a separate locking discipline than regular bins.
	 * Because they are forwarded via special MOVED nodes at bin heads (which
	 * can never change once established), we cannot use those nodes as locks.
	 * Instead, TreeBin extends AbstractQueuedSynchronizer to support a simple
	 * form of read-write lock. For update operations and table validation, the
	 * exclusive form of lock behaves in the same way as bin-head locks.
	 * However, lookups use shared read-lock mechanics to allow multiple readers
	 * in the absence of writers. Additionally, these lookups do not ever block:
	 * While the lock is not available, they proceed along the slow traversal
	 * path (via next-pointers) until the lock becomes available or the list is
	 * exhausted, whichever comes first. (These cases are not fast, but maximize
	 * aggregate expected throughput.) The AQS mechanics for doing this are
	 * straightforward. The lock state is held as AQS getState(). Read counts
	 * are negative; the write count (1) is positive. There are no signalling
	 * preferences among readers and writers. Since we don't need to export full
	 * Lock API, we just override the minimal AQS methods and use them directly.
	 */
	static final class TreeBin<V> extends AbstractQueuedSynchronizer {
		...
		public final boolean tryReleaseShared(int ignore) {
			int c;
			do {
			} while (!compareAndSetState(c = getState(), c + 1));
			return c == -1;
		}
			...
			while (p != null) {
				...
				if ((ph = p.hash) == h) {
					if ((pk = p.key) == k || k.equals(pk))
						return p;
					if (c != (pc = pk.getClass()) || !(k instanceof Comparable)
							|| (dir = ((Comparable<Comparable<?>>) k).compareTo((Comparable<?>) pk)) == 0) {
		...
		/**
		 * Wrapper for getTreeNode used by CHM.get. Tries to obtain read-lock to
		 * call getTreeNode, but during failure to get lock, searches along next
		 * links.
		 */
		...
		final void deleteTreeNode(TreeNode<V> p) {
			TreeNode<V> next = (TreeNode<V>) p.next; // unlink traversal
														// pointers
	...
	/**
	 * Spreads higher bits to lower, and also forces top bit to 0. Because the
	 * table uses power-of-two masking, sets of hashes that vary only in bits
	 * above the current mask will always collide. (Among known examples are
	 * sets of Float keys holding consecutive whole numbers in small tables.) To
	 * counter this, we apply a transform that spreads the impact of higher bits
	 * downward. There is a tradeoff between speed, utility, and quality of
	 * bit-spreading. Because many common sets of hashes are already reasonably
	 * distributed across bits (so don't benefit from spreading), and because we
	 * use trees to handle large sets of collisions in bins, we don't need
	 * excessively high quality.
	 */
	...
	/**
	 * Replaces a list bin with a tree bin if key is comparable. Call only when
	 * locked.
	 */
	...
	/*
	 * Internal versions of insertion methods All have the same basic structure
	 * as the first (internalPut): 1. If table uninitialized, create 2. If bin
	 * empty, try to CAS new node 3. If bin stale, use new table 4. if bin
	 * converted to TreeBin, validate and relay to TreeBin methods 5. Lock and
	 * validate; if valid, scan and add or update
	 *
	 * The putAll method differs mainly in attempting to pre-allocate enough
	 * table space, and also more lazily performs count updates and checks.
	 *
	 * Most of the function-accepting methods can't be factored nicely because
	 * they require different functional forms, so instead sprawl out similar
	 * mechanics.
	 */
	/**
		...
		for (Node<V>[] tab = table;;) {
			...
			if (tab == null)
				...
				if ((fk = f.key) instanceof TreeBin) {
					...
					tab = (Node<V>[]) fk;
			} else if (onlyIfAbsent && fh == h && (fv = f.val) != null && ((fk = f.key) == k || k.equals(fk))) // peek
																												// while
																												// nearby
				return fv;
			else {
					...
					if (tab == null)
						...
						synchronized (f) {
							if (tabAt(tab, i) == f) {
								...
								for (Node<V> e = f;; ++len) {
									Object ek;
									if (e.hash == h && (e.val) != null && ((ek = e.key) == k || k.equals(ek))) {
										e.val = v;
										break;
									}
									Node<V> last = e;
		...
		for (;;) {
			...
			if ((as = counterCells) != null && (n = as.length) > 0) {
				if ((a = as[(n - 1) & h]) == null) {
					...
					collide = true;
				else if (counterBusy == 0 && U.compareAndSwapInt(this, COUNTERBUSY, 0, 1)) {
					try {
	 ...
	 * a base class for other iterators and bulk tasks.
	 *
	 * At each step, the iterator snapshots the key ("nextKey") and value
	 * ("nextVal") of a valid node (i.e., one that, at point of snapshot, has a
	 * non-null user value). Because val fields can change (including to null,
	 * indicating deletion), field nextVal might not be accurate at point of
	 * use, but still maintains the weak consistency property of holding a value
	 * that was once valid. To support iterator.remove, the nextKey field is not
	 * updated (nulled out) when the iterator cannot advance.
	 *
	 * Internal traversals directly access these fields, as in: {@code while
	 * (it.advance() != null) { process(it.nextKey); }}
	 *
	 * Exported iterators must track whether the iterator has advanced (in
	 * hasNext vs next) (by setting/checking/nulling field nextVal), and then
	 * extract key, value, or key-value pairs as return values of next().
	 *
	 * The iterator visits once each still-valid node that was reachable upon
	 ...
	 *
	 * This class extends CountedCompleter to streamline parallel iteration in
	 * bulk operations. This adds only a few fields of space overhead, which is
	 * small enough in cases where it is not needed to not worry about it.
	 * Because CountedCompleter is Serializable, but iterators need not be, we
	...
	static class Traverser<K, V, R> extends CountedCompleter<R> {
		...
		Traverser(ConcurrentHashMapV8<K, V> map, Traverser<K, V, ?> it, int batch) {
			...
			if ((this.map = map) != null && it != null) { // split parent
				Node<V>[] t;
				if ((t = it.tab) == null && (t = it.tab = map.table) != null)
					it.baseLimit = it.baseSize = t.length;
				this.tab = t;
				this.baseSize = it.baseSize;
				int hi = this.baseLimit = it.baseLimit;
				it.baseLimit = this.index = this.baseIndex = (hi + it.baseIndex + 1) >>> 1;
			}
		}

		/**
		 * Advances next; returns nextVal or null if terminated. See above for
		 * explanation.
		 */
		@SuppressWarnings("unchecked")
		final V advance() {
			...
			outer: do {
				...
				while (e == null) { // get to next non-null bin
					...
					if ((t = tab) != null)
						...
						break outer;
					if ((b = baseIndex) >= baseLimit || (i = index) < 0 || i >= n)
						break outer;
		...
		@Override
		public void compute() {
		} // default no-op CountedCompleter body

		/**
		 * Returns a batch value > 0 if this task should (and must) be split, if
		 * so, adding to pending count, and in any case updating batch value.
		 * The initial batch value is approx exp2 of the number of times (minus
		 * one) to split task by two before executing leaf action. This value is
		 * faster to compute and more convenient to use as a guide to splitting
		 * than is the depth, since it is used while dividing by two anyway.
		 */
		final int preSplit() {
			ConcurrentHashMapV8<K, V> m;
			int b;
			...
			if ((b = batch) < 0 && (m = map) != null) { // force initialization
				...
				if (t != null) {
					long n = m.sumCount();
					int par = ((pool = getPool()) == null) ? ForkJoinPool.getCommonPoolParallelism()
							: pool.getParallelism();
	...
	/**
	 * Creates a new {@link Set} backed by a ConcurrentHashMapV8 from the given
	 ...
	 * {@link #size} because a ConcurrentHashMapV8 may contain more mappings
	 * than can be represented as an int. The value returned is an estimate; the
	 ...
	 * </pre>
	 *
	 * except that the action is performed atomically. If the function returns
	 * {@code null} no mapping is recorded. If the function itself throws an
	 * (unchecked) exception, the exception is rethrown to its caller, and no
	 * mapping is recorded. Some attempted update operations on this map by
	 * other threads may be blocked while computation is in progress, so the
	 * computation should be short and simple, and must not attempt to update
	 * any other mappings of this Map. The most appropriate usage is to
	 ...
	 *
	 * except that the action is performed atomically. If the function returns
	 * {@code null}, the mapping is removed. If the function itself throws an
	 * (unchecked) exception, the exception is rethrown to its caller, and the
	 * current mapping is left unchanged. Some attempted update operations on
	 ...
	 * attempt to update any other mappings of this Map. For example, to either
	 * create or append new messages to a value mapping:
	 *
	 * @param key
	 *            key with which the specified value is to be associated
	 ...
	 * {@code null} if there is no current mapping). This is equivalent to
	 * 
	 * <pre>
	 *  {@code
	 *   value = remappingFunction.apply(key, map.get(key));
	 ...
	 * with the given value. Otherwise, replace the value with the results of
	 ...
	 * {@link Collection#addAll}). This is of course only appropriate if it is
	...
	public boolean equals(Object o) {
		if (o != this) {
			...
			for (Map.Entry<?, ?> e : m.entrySet()) {
				Object mk, mv, v;
				if ((mk = e.getKey()) == null || (mv = e.getValue()) == null || (v = internalGet(mk)) == null
						|| (mv != v && !mv.equals(v)))
					return false;
			}
		}
	...
	/**
	 * Returns exportable snapshot entry for the given key and value when
	 * write-through can't or shouldn't be used.
	 */
	...
	/**
	 * Saves the state of the {@code ConcurrentHashMapV8} instance to a stream
	 * (i.e., serializes it).
	 *
	 * @param s
	 *            the stream
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <U> U reduceSequentially(BiFun<? super K, ? super V, ? extends U> transformer,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceToDoubleSequentially(ObjectByObjectToDouble<? super K, ? super V> transformer, double basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceToLongSequentially(ObjectByObjectToLong<? super K, ? super V> transformer, long basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public int reduceToIntSequentially(ObjectByObjectToInt<? super K, ? super V> transformer, int basis,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <U> U reduceKeysSequentially(Fun<? super K, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceKeysToDoubleSequentially(ObjectToDouble<? super K> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceKeysToLongSequentially(ObjectToLong<? super K> transformer, long basis,
			LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	public <U> U reduceValuesSequentially(Fun<? super V, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public double reduceValuesToDoubleSequentially(ObjectToDouble<? super V> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public long reduceValuesToLongSequentially(ObjectToLong<? super V> transformer, long basis,
			LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <U> U reduceEntriesSequentially(Fun<Map.Entry<K, V>, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public double reduceEntriesToDoubleSequentially(ObjectToDouble<Map.Entry<K, V>> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public long reduceEntriesToLongSequentially(ObjectToLong<Map.Entry<K, V>> transformer, long basis,
			LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public int reduceEntriesToIntSequentially(ObjectToInt<Map.Entry<K, V>> transformer, int basis,
			IntByIntToInt reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
		Traverser<K, V, Object> it = new Traverser<K, V, Object>(this);
	 ...
	 *         value) pairs
	 */
	public <U> U reduceInParallel(BiFun<? super K, ? super V, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
	 ...
	 *         value) pairs
	 */
	public double reduceToDoubleInParallel(ObjectByObjectToDouble<? super K, ? super V> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
	 ...
	 *         value) pairs
	 */
	public long reduceToLongInParallel(ObjectByObjectToLong<? super K, ? super V> transformer, long basis,
			LongByLongToLong reducer) {
	 ...
	 *         value) pairs
	 */
	public int reduceToIntInParallel(ObjectByObjectToInt<? super K, ? super V> transformer, int basis,
			IntByIntToInt reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all keys
	 */
	public <U> U reduceKeysInParallel(Fun<? super K, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all keys
	 */
	public double reduceKeysToDoubleInParallel(ObjectToDouble<? super K> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public <U> U reduceValuesInParallel(Fun<? super V, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public double reduceValuesToDoubleInParallel(ObjectToDouble<? super V> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
	 ...
	 * @return the result of accumulating the given transformation of all values
	 */
	public long reduceValuesToLongInParallel(ObjectToLong<? super V> transformer, long basis,
			LongByLongToLong reducer) {
	 ...
	 *         entries
	 */
	public <U> U reduceEntriesInParallel(Fun<Map.Entry<K, V>, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
	 ...
	 *         entries
	 */
	public double reduceEntriesToDoubleInParallel(ObjectToDouble<Map.Entry<K, V>> transformer, double basis,
			DoubleByDoubleToDouble reducer) {
	 ...
	 *         entries
	 */
	public long reduceEntriesToLongInParallel(ObjectToLong<Map.Entry<K, V>> transformer, long basis,
			LongByLongToLong reducer) {
	 ...
	 *         entries
	 */
	public int reduceEntriesToIntInParallel(ObjectToInt<Map.Entry<K, V>> transformer, int basis,
			IntByIntToInt reducer) {
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of keys, in which
	 * additions may optionally be enabled by mapping to a common value. This
	 * class cannot be directly instantiated. See {@link #keySet()},
	 * {@link #keySet(Object)}, {@link #newKeySet()}, {@link #newKeySet(int)}.
	 */
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Collection} of values, in
	 * which additions are disabled. This class cannot be directly instantiated.
	 * See {@link #values()}.
	 *
	 * <p>
	 * The view's {@code iterator} is a "weakly consistent" iterator that will
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of (key, value) entries.
	 ...
	 * ConcurrentHashMapV8s. These tasks follow the forms and rules used for
	 * bulk operations. Each method has the same name, but returns a task rather
	 * than invoking it. These methods may be useful in custom applications such
	 * as submitting a task without waiting for completion, using a custom pool,
	 * or combining with other tasks.
	 */
	public static class ForkJoinTasks {
		 ...
		 * @param transformer
		 *            a function returning the transformation for an element, or
		 *            null if there is no transformation (in which case the
		...
		public static <K, V, U> ForkJoinTask<Void> forEach(ConcurrentHashMapV8<K, V> map,
				BiFun<? super K, ? super V, ? extends U> transformer, Action<U> action) {
			if (transformer == null || action == null)
				throw new NullPointerException();
		 ...
		 * none. Upon success, further element processing is suppressed and the
		 * results of any other parallel invocations of the search function are
		 * ignored.
		 *
		 * @param map
		 *            the map
		 * @param searchFunction
		 *            a function returning a non-null result on success, else
		 *            null
		 * @return the task
		 */
		public static <K, V, U> ForkJoinTask<U> search(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchMappingsTask<K, V, U>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		 ...
		 * @param transformer
		 *            a function returning the transformation for an element, or
		 *            null if there is no transformation (in which case it is
		 *            not combined)
		 * @param reducer
		 *            a commutative associative combining function
		 * @return the task
		...
		public static <K, V> ForkJoinTask<Double> reduceToDouble(ConcurrentHashMapV8<K, V> map,
				ObjectByObjectToDouble<? super K, ? super V> transformer, double basis,
				DoubleByDoubleToDouble reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Long> reduceToLong(ConcurrentHashMapV8<K, V> map,
				ObjectByObjectToLong<? super K, ? super V> transformer, long basis, LongByLongToLong reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Integer> reduceToInt(ConcurrentHashMapV8<K, V> map,
				ObjectByObjectToInt<? super K, ? super V> transformer, int basis, IntByIntToInt reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		/**
		 * Returns a task that when invoked, returns a non-null result from
		 * applying the given search function on each key, or null if none. Upon
		 * success, further element processing is suppressed and the results of
		 * any other parallel invocations of the search function are ignored.
		 *
		 * @param map
		 *            the map
		 * @param searchFunction
		...
		public static <K, V, U> ForkJoinTask<U> searchKeys(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchKeysTask<K, V, U>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceKeys(ConcurrentHashMapV8<K, V> map,
				Fun<? super K, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Double> reduceKeysToDouble(ConcurrentHashMapV8<K, V> map,
				ObjectToDouble<? super K> transformer, double basis, DoubleByDoubleToDouble reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Long> reduceKeysToLong(ConcurrentHashMapV8<K, V> map,
				ObjectToLong<? super K> transformer, long basis, LongByLongToLong reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Integer> reduceKeysToInt(ConcurrentHashMapV8<K, V> map,
				ObjectToInt<? super K> transformer, int basis, IntByIntToInt reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		 ...
		 * Upon success, further element processing is suppressed and the
		 * results of any other parallel invocations of the search function are
		 * ignored.
		 *
		 * @param map
		...
		public static <K, V, U> ForkJoinTask<U> searchValues(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchValuesTask<K, V, U>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceValues(ConcurrentHashMapV8<K, V> map,
				Fun<? super V, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Double> reduceValuesToDouble(ConcurrentHashMapV8<K, V> map,
				ObjectToDouble<? super V> transformer, double basis, DoubleByDoubleToDouble reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Long> reduceValuesToLong(ConcurrentHashMapV8<K, V> map,
				ObjectToLong<? super V> transformer, long basis, LongByLongToLong reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Integer> reduceValuesToInt(ConcurrentHashMapV8<K, V> map,
				ObjectToInt<? super V> transformer, int basis, IntByIntToInt reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V, U> ForkJoinTask<U> searchEntries(ConcurrentHashMapV8<K, V> map,
			...
			if (searchFunction == null)
				throw new NullPointerException();
			return new SearchEntriesTask<K, V, U>(map, null, -1, searchFunction, new AtomicReference<U>());
		}
		...
		public static <K, V, U> ForkJoinTask<U> reduceEntries(ConcurrentHashMapV8<K, V> map,
				Fun<Map.Entry<K, V>, ? extends U> transformer, BiFun<? super U, ? super U, ? extends U> reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Double> reduceEntriesToDouble(ConcurrentHashMapV8<K, V> map,
				ObjectToDouble<Map.Entry<K, V>> transformer, double basis, DoubleByDoubleToDouble reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Long> reduceEntriesToLong(ConcurrentHashMapV8<K, V> map,
				ObjectToLong<Map.Entry<K, V>> transformer, long basis, LongByLongToLong reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
		...
		public static <K, V> ForkJoinTask<Integer> reduceEntriesToInt(ConcurrentHashMapV8<K, V> map,
				ObjectToInt<Map.Entry<K, V>> transformer, int basis, IntByIntToInt reducer) {
			if (transformer == null || reducer == null)
				throw new NullPointerException();
	...
	static final class ForEachTransformedKeyTask<K, V, U> extends Traverser<K, V, Void> {
		...
		ForEachTransformedKeyTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<? super K, ? extends U> transformer, Action<U> action) {
			super(m, p, b);
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (action = this.action) != null) {
				for (int b; (b = preSplit()) > 0;)
					new ForEachTransformedKeyTask<K, V, U>(map, this, b, transformer, action).fork();
				U u;
				while (advance() != null) {
	...
	static final class ForEachTransformedValueTask<K, V, U> extends Traverser<K, V, Void> {
		...
		ForEachTransformedValueTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<? super V, ? extends U> transformer, Action<U> action) {
			super(m, p, b);
			this.transformer = transformer;
			this.action = action;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (action = this.action) != null) {
				for (int b; (b = preSplit()) > 0;)
					new ForEachTransformedValueTask<K, V, U>(map, this, b, transformer, action).fork();
				V v;
				U u;
				while ((v = advance()) != null) {
					if ((u = transformer.apply(v)) != null)
	...
	static final class ForEachTransformedEntryTask<K, V, U> extends Traverser<K, V, Void> {
		...
		ForEachTransformedEntryTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<Map.Entry<K, V>, ? extends U> transformer, Action<U> action) {
			super(m, p, b);
			this.transformer = transformer;
			this.action = action;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (action = this.action) != null) {
				for (int b; (b = preSplit()) > 0;)
					new ForEachTransformedEntryTask<K, V, U>(map, this, b, transformer, action).fork();
				V v;
				U u;
				while ((v = advance()) != null) {
					if ((u = transformer.apply(entryFor((K) nextKey, v))) != null)
						action.apply(u);
				}
				propagateCompletion();
			}
	...
	static final class ForEachTransformedMappingTask<K, V, U> extends Traverser<K, V, Void> {
		...
		ForEachTransformedMappingTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				BiFun<? super K, ? super V, ? extends U> transformer, Action<U> action) {
			super(m, p, b);
			this.transformer = transformer;
			this.action = action;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (action = this.action) != null) {
				for (int b; (b = preSplit()) > 0;)
					new ForEachTransformedMappingTask<K, V, U>(map, this, b, transformer, action).fork();
				V v;
				U u;
				while ((v = advance()) != null) {
	...
	static final class SearchKeysTask<K, V, U> extends Traverser<K, V, U> {
		...
		SearchKeysTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<? super K, ? extends U> searchFunction, AtomicReference<U> result) {
			super(m, p, b);
	...
	static final class SearchValuesTask<K, V, U> extends Traverser<K, V, U> {
		...
		SearchValuesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<? super V, ? extends U> searchFunction, AtomicReference<U> result) {
			super(m, p, b);
			this.searchFunction = searchFunction;
			this.result = result;
		}
		...
		public final void compute() {
			...
			if ((searchFunction = this.searchFunction) != null && (result = this.result) != null) {
				for (int b;;) {
					...
					if ((b = preSplit()) <= 0)
						break;
					new SearchValuesTask<K, V, U>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null) {
					V v;
					U u;
	...
	static final class SearchEntriesTask<K, V, U> extends Traverser<K, V, U> {
		...
		SearchEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				Fun<Entry<K, V>, ? extends U> searchFunction, AtomicReference<U> result) {
			super(m, p, b);
			this.searchFunction = searchFunction;
			this.result = result;
		}
	...
	static final class SearchMappingsTask<K, V, U> extends Traverser<K, V, U> {
		...
		SearchMappingsTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				BiFun<? super K, ? super V, ? extends U> searchFunction, AtomicReference<U> result) {
			super(m, p, b);
			this.searchFunction = searchFunction;
			this.result = result;
		}
		...
		public final void compute() {
			...
			if ((searchFunction = this.searchFunction) != null && (result = this.result) != null) {
				for (int b;;) {
					...
					if ((b = preSplit()) <= 0)
						break;
					new SearchMappingsTask<K, V, U>(map, this, b, searchFunction, result).fork();
				}
				while (result.get() == null) {
					V v;
					U u;
	...
	static final class ReduceKeysTask<K, V> extends Traverser<K, V, K> {
		...
		ReduceKeysTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b, ReduceKeysTask<K, V> nextRight,
				BiFun<? super K, ? super K, ? extends K> reducer) {
			super(m, p, b);
		...
		public final void compute() {
			...
			if ((reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new ReduceKeysTask<K, V>(map, this, b, rights, reducer)).fork();
				K r = null;
				while (advance() != null) {
					K u = (K) nextKey;
					r = (r == null) ? u : reducer.apply(r, u);
	...
	static final class ReduceValuesTask<K, V> extends Traverser<K, V, V> {
		...
		ReduceValuesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b, ReduceValuesTask<K, V> nextRight,
				BiFun<? super V, ? super V, ? extends V> reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.reducer = reducer;
		...
		public final void compute() {
			...
			if ((reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new ReduceValuesTask<K, V>(map, this, b, rights, reducer)).fork();
				V r = null;
				V v;
				while ((v = advance()) != null) {
					V u = v;
	...
	static final class ReduceEntriesTask<K, V> extends Traverser<K, V, Map.Entry<K, V>> {
		...
		ReduceEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b, ReduceEntriesTask<K, V> nextRight,
		...
		public final void compute() {
			...
			if ((reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new ReduceEntriesTask<K, V>(map, this, b, rights, reducer)).fork();
				Map.Entry<K, V> r = null;
				V v;
				while ((v = advance()) != null) {
	...
	static final class MapReduceKeysTask<K, V, U> extends Traverser<K, V, U> {
		...
		MapReduceKeysTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceKeysTask<K, V, U> nextRight, Fun<? super K, ? extends U> transformer,
				BiFun<? super U, ? super U, ? extends U> reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceKeysTask<K, V, U>(map, this, b, rights, transformer, reducer)).fork();
				U r = null, u;
				while (advance() != null) {
					if ((u = transformer.apply((K) nextKey)) != null)
						r = (r == null) ? u : reducer.apply(r, u);
	...
	static final class MapReduceValuesTask<K, V, U> extends Traverser<K, V, U> {
		...
		MapReduceValuesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceValuesTask<K, V, U> nextRight, Fun<? super V, ? extends U> transformer,
				BiFun<? super U, ? super U, ? extends U> reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesTask<K, V, U>(map, this, b, rights, transformer, reducer)).fork();
				U r = null, u;
				V v;
				while ((v = advance()) != null) {
					if ((u = transformer.apply(v)) != null)
	...
	static final class MapReduceEntriesTask<K, V, U> extends Traverser<K, V, U> {
		...
		MapReduceEntriesTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceEntriesTask<K, V, U> nextRight, Fun<Map.Entry<K, V>, ? extends U> transformer,
				BiFun<? super U, ? super U, ? extends U> reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesTask<K, V, U>(map, this, b, rights, transformer, reducer)).fork();
				U r = null, u;
				V v;
				while ((v = advance()) != null) {
					if ((u = transformer.apply(entryFor((K) nextKey, v))) != null)
	...
	static final class MapReduceMappingsTask<K, V, U> extends Traverser<K, V, U> {
		...
		MapReduceMappingsTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceMappingsTask<K, V, U> nextRight, BiFun<? super K, ? super V, ? extends U> transformer,
				BiFun<? super U, ? super U, ? extends U> reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsTask<K, V, U>(map, this, b, rights, transformer, reducer)).fork();
				U r = null, u;
				V v;
				while ((v = advance()) != null) {
					if ((u = transformer.apply((K) nextKey, v)) != null)
	...
	static final class MapReduceKeysToDoubleTask<K, V> extends Traverser<K, V, Double> {
		...
		MapReduceKeysToDoubleTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceKeysToDoubleTask<K, V> nextRight, ObjectToDouble<? super K> transformer, double basis,
				DoubleByDoubleToDouble reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceKeysToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				while (advance() != null)
	...
	static final class MapReduceValuesToDoubleTask<K, V> extends Traverser<K, V, Double> {
		...
		MapReduceValuesToDoubleTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceValuesToDoubleTask<K, V> nextRight, ObjectToDouble<? super V> transformer, double basis,
				DoubleByDoubleToDouble reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(v));
	...
	static final class MapReduceEntriesToDoubleTask<K, V> extends Traverser<K, V, Double> {
		...
		MapReduceEntriesToDoubleTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceEntriesToDoubleTask<K, V> nextRight, ObjectToDouble<Map.Entry<K, V>> transformer, double basis,
				DoubleByDoubleToDouble reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(entryFor((K) nextKey, v)));
				result = r;
				CountedCompleter<?> c;
				for (c = firstComplete(); c != null; c = c.nextComplete()) {
	...
	static final class MapReduceMappingsToDoubleTask<K, V> extends Traverser<K, V, Double> {
		...
		MapReduceMappingsToDoubleTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceMappingsToDoubleTask<K, V> nextRight, ObjectByObjectToDouble<? super K, ? super V> transformer,
				double basis, DoubleByDoubleToDouble reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
			this.basis = basis;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				double r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToDoubleTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
	...
	static final class MapReduceKeysToLongTask<K, V> extends Traverser<K, V, Long> {
		...
		MapReduceKeysToLongTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceKeysToLongTask<K, V> nextRight, ObjectToLong<? super K> transformer, long basis,
				LongByLongToLong reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceKeysToLongTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				while (advance() != null)
					r = reducer.apply(r, transformer.apply((K) nextKey));
				result = r;
				CountedCompleter<?> c;
	...
	static final class MapReduceValuesToLongTask<K, V> extends Traverser<K, V, Long> {
		...
		MapReduceValuesToLongTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceValuesToLongTask<K, V> nextRight, ObjectToLong<? super V> transformer, long basis,
				LongByLongToLong reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesToLongTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(v));
	...
	static final class MapReduceEntriesToLongTask<K, V> extends Traverser<K, V, Long> {
		...
		MapReduceEntriesToLongTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceEntriesToLongTask<K, V> nextRight, ObjectToLong<Map.Entry<K, V>> transformer, long basis,
				LongByLongToLong reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToLongTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(entryFor((K) nextKey, v)));
	...
	static final class MapReduceMappingsToLongTask<K, V> extends Traverser<K, V, Long> {
		...
		MapReduceMappingsToLongTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceMappingsToLongTask<K, V> nextRight, ObjectByObjectToLong<? super K, ? super V> transformer,
				long basis, LongByLongToLong reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
			this.basis = basis;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				long r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToLongTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply((K) nextKey, v));
	...
	static final class MapReduceKeysToIntTask<K, V> extends Traverser<K, V, Integer> {
		...
		MapReduceKeysToIntTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceKeysToIntTask<K, V> nextRight, ObjectToInt<? super K> transformer, int basis,
				IntByIntToInt reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceKeysToIntTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				while (advance() != null)
					r = reducer.apply(r, transformer.apply((K) nextKey));
				result = r;
				CountedCompleter<?> c;
	...
	static final class MapReduceValuesToIntTask<K, V> extends Traverser<K, V, Integer> {
		...
		MapReduceValuesToIntTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceValuesToIntTask<K, V> nextRight, ObjectToInt<? super V> transformer, int basis,
				IntByIntToInt reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceValuesToIntTask<K, V>(map, this, b, rights, transformer, r, reducer)).fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(v));
				result = r;
	...
	static final class MapReduceEntriesToIntTask<K, V> extends Traverser<K, V, Integer> {
		...
		MapReduceEntriesToIntTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceEntriesToIntTask<K, V> nextRight, ObjectToInt<Map.Entry<K, V>> transformer, int basis,
				IntByIntToInt reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceEntriesToIntTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply(entryFor((K) nextKey, v)));
	...
	static final class MapReduceMappingsToIntTask<K, V> extends Traverser<K, V, Integer> {
		...
		MapReduceMappingsToIntTask(ConcurrentHashMapV8<K, V> m, Traverser<K, V, ?> p, int b,
				MapReduceMappingsToIntTask<K, V> nextRight, ObjectByObjectToInt<? super K, ? super V> transformer,
				int basis, IntByIntToInt reducer) {
			super(m, p, b);
			this.nextRight = nextRight;
			this.transformer = transformer;
			this.basis = basis;
		...
		public final void compute() {
			...
			if ((transformer = this.transformer) != null && (reducer = this.reducer) != null) {
				int r = this.basis;
				for (int b; (b = preSplit()) > 0;)
					(rights = new MapReduceMappingsToIntTask<K, V>(map, this, b, rights, transformer, r, reducer))
							.fork();
				V v;
				while ((v = advance()) != null)
					r = reducer.apply(r, transformer.apply((K) nextKey, v));
