 ...
 *
 * <p>
 * A "main" {@code ForkJoinTask} begins execution when it is explicitly
 * submitted to a {@link ForkJoinPool}, or, if not already engaged in a ForkJoin
 * computation, commenced in the {@link ForkJoinPool#commonPool()} via
 * {@link #fork}, {@link #invoke}, or related methods. Once started, it will
 * usually in turn start other subtasks. As indicated by the name of this class,
 * many programs using {@code ForkJoinTask} employ only methods {@link #fork}
 * and {@link #join}, or derivatives such as {@link #invokeAll(ForkJoinTask...)
 ...
 *
 * <p>
 * A {@code ForkJoinTask} is a lightweight form of {@link Future}. The
 ...
 * {@link RejectedExecutionException} stemming from internal resource
 ...
 * requires three further considerations: (1) Completion of few if any
 ...
 * performing only the (possibly) blocking action. (3) Unless the
 * {@link ForkJoinPool.ManagedBlocker} API is used, or the number of possibly
 * blocked tasks is known to be less than the pool's
 * {@link ForkJoinPool#getParallelism} level, the pool cannot guarantee that
 * enough threads will be available to ensure progress or good performance.
 *
 * <p>
 * The primary method for awaiting completion and extracting results of a task
 * is {@link #join}, but there are several variants: The {@link Future#get}
 ...
 * either the encountered exception or
 ...
 * fork/join processing, typically {@link RecursiveAction} for most computations
 * that do not return results, {@link RecursiveTask} for those that do, and
 * {@link CountedCompleter} for those in which completed actions trigger other
 * actions. Normally, a concrete ForkJoinTask subclass declares fields
 * comprising its parameters, established in a constructor, and then defines a
 * {@code compute} method that somehow uses the control methods supplied by this
 * base class.
 *
 * <p>
 * Method {@link #join} and its variants are appropriate for use only when
 ...
 * using {@link #setForkJoinTaskTag} or {@link #compareAndSetForkJoinTaskTag}
 * and checked using {@link #getForkJoinTaskTag}. The ForkJoinTask
 * implementation does not use these {@code protected} methods or tags for any
 * purpose, but they may be of use in the construction of specialized
 * subclasses. For example, parallel graph traversals can use the supplied
 * methods to avoid revisiting nodes/tasks that have already been processed.
 * (Method names for tagging are bulky in part to encourage definition of
 * methods that reflect their usage patterns.)
 *
 * <p>
 * Most base support methods are {@code final}, to prevent overriding of
 ...
 * processing should minimally implement {@code protected} methods
 * {@link #exec}, {@link #setRawResult}, and {@link #getRawResult}, while also
 ...
 *
 * <p>
 * This class provides {@code adapt} methods for {@link Runnable} and
 ...
 *
 * <p>
 * ForkJoinTasks are {@code Serializable}, which enables them to be used in
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
	...
	/*
	 * See the internal documentation of class ForkJoinPool for a general
	 * implementation overview. ForkJoinTasks are mainly responsible for
	 * maintaining their "status" field amidst relays to methods in
	 * ForkJoinWorkerThread and ForkJoinPool.
	 *
	 * The methods of this class are more-or-less layered into (1) basic status
	 * maintenance (2) execution and awaiting completion (3) user-level methods
	 * that additionally report results. This is sometimes hard to see because
	 * this file orders exported methods in a way that flows well in javadocs.
	 */

	/*
	 * The status field holds run control status bits packed into a single int
	 * to minimize footprint and to ensure atomicity (via CAS). Status is
	 * initially zero, and takes on nonnegative values until completed, upon
	 * which status (anded with DONE_MASK) holds value NORMAL, CANCELLED, or
	 * EXCEPTIONAL. Tasks undergoing blocking waits by other threads have the
	 * SIGNAL bit set. Completion of a stolen task with SIGNAL set awakens any
	 * waiters via notifyAll. Even though suboptimal for some purposes, we use
	 * basic builtin wait/notify to take advantage of "monitor inflation" in
	 * JVMs that we would otherwise need to emulate to avoid adding further
	 * per-task bookkeeping overhead. We want these monitors to be "fat", i.e.,
	 * not use biasing or thin-lock techniques, so use some odd coding idioms
	 * that tend to avoid them, mainly by arranging that every synchronized
	 * block performs a wait, notifyAll or both.
	 *
	 * These control bits occupy only (some of) the upper half (16 bits) of
	...
	private int doInvoke() {
		...
		return (s = doExec()) < 0 ? s
				: ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
						? (wt = (ForkJoinWorkerThread) t).pool.awaitJoin(wt.workQueue, this) : externalAwaitDone();
	}
	 ...
	 * {@link ForkJoinPool#commonPool()} if not {@link #inForkJoinPool}. While
	 * it is not necessarily enforced, it is a usage error to fork a task more
	 * than once unless it has completed and been reinitialized. Subsequent
	 ...
	 * This method differs from {@link #get()} in that abnormal completion
	 * results in {@code RuntimeException} or {@code Error}, not
	 ...
	 * exceptional return. The status of each task may be obtained using
	 * {@link #getException()} and related methods to check if they have been
	 ...
	 * returns successfully, unless there is an intervening call to
	 * {@link #reinitialize}, subsequent calls to {@link #isCancelled},
	 * {@link #isDone}, and {@code cancel} will return {@code true} and calls to
	 * {@link #join} and related methods will result in
	 * {@code CancellationException}.
	 *
	 * <p>
	 * This method may be overridden in subclasses, but if so, must still ensure
	 * that these properties hold. In particular, the {@code cancel} method
	 * itself must not throw exceptions.
	 *
	 * <p>
	 * This method is designed to be invoked by <em>other</em> tasks. To
	 * terminate the current task, you can just return or throw an unchecked
	 * exception from its computation method, or invoke
	 ...
	 *            {@code RuntimeException} or {@code Error}, the actual
	 *            exception thrown will be a {@code RuntimeException} with cause
	 *            {@code ex}.
	 */
	 ...
	 *             if the current thread is not a member of a ForkJoinPool and
	 ...
	 *
	 * <p>
	 * Upon completion of this method, {@code isDone()} reports {@code false},
	 * and {@code getException()} reports {@code
	...
	/**
	 * Returns {@code true} if the current thread is a
	 * {@link ForkJoinWorkerThread} executing as a ForkJoinPool computation.
	 *
	 * @return {@code true} if the current thread is a
	 *         {@link ForkJoinWorkerThread} executing as a ForkJoinPool
	 *         computation, or {@code false} otherwise
	 */
	...
	public boolean tryUnfork() {
		...
		return (((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
				? ((ForkJoinWorkerThread) t).workQueue.tryUnpush(this) : ForkJoinPool.tryExternalUnpush(this));
	}
	...
	protected static ForkJoinTask<?> pollTask() {
		...
		ForkJoinWorkerThread wt;
		return ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
				? (wt = (ForkJoinWorkerThread) t).pool.nextTaskFor(wt.workQueue) : null;
	}
